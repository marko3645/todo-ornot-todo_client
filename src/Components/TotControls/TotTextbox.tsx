...
interface TotTextboxState extends IInputControl {
	Text?: string;
	Type?: "text" | "password";
	OnChange?: (val: string | number, key: any, event: React.SyntheticEvent<HTMLInputElement, Event>) => void;
}
...
export class TotTextbox extends React.Component<TotTextboxState, TotTextboxState> {
	//Component variables
	private TextboxRef: React.RefObject<HTMLInputElement> = React.createRef<HTMLInputElement>();
	...
	private BuildComponent(formContext: FormContext) {
		...
		innerContainerClassNames += this.state.HasFocus || this.state.Text ? " active" : "";
		innerContainerClassNames += this.state.disabled || formContext?.disabled ? " disabled" : "";
		return (
	...
	private BuildLabel(formContext: FormContext) {
		...
		return (
			<Label className={labelClasses} onClick={() => this.OnLabel_Click(formContext)}>
	...
	private OnChange(event: React.SyntheticEvent<HTMLInputElement, Event>) {
		if (this.props.OnChange) {
			this.props.OnChange((event.target as HTMLInputElement).value, this.props.name, event);
		}
	}
}
